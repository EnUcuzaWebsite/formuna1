                ...
                <div class="flex-1">
                    ...
                    <div class="flex flex-wrap gap-2 mt-1">
                        @foreach($category->mostSharedTopics(count: 3) as $topic)
                            <a href="{{ route('topic.detail', $topic) }}" wire:navigate class="text-indigo-500 text-sm font-medium">#{{ $topic->name }}</a>
                        @endforeach
                    </div>
                    <span class="text-gray-500 text-sm mt-1"> {{ $category->posts->count() }} Gönderi</span>
                </div>
                <a href="{{ route('category.detail', $category) }}"
                class="absolute right-5 text-sm text-indigo-400 bottom-1/2 flex items-center p-1 gap-2 cursor-pointer" wire:navigate>
                    <span>Görüntüle</span>
